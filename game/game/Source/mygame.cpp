...
void CGameStateRun::OnBeginState()
{
	...
	const int ZOMBIE_GAP = 192;
	const int ZOMBIE_X_OFFSET = 492;
	const int ZOMBIE_Y_OFFSET = 110;
	const int ZOMBIE_PER_ROW = 3;

	for (int i = 0; i < 1; i++) {
		int x_pos = i % ZOMBIE_PER_ROW;
		int y_pos = i / ZOMBIE_PER_ROW;
		zombie[i].SetXY(x_pos * ZOMBIE_GAP + ZOMBIE_X_OFFSET, y_pos *ZOMBIE_GAP + ZOMBIE_Y_OFFSET);
		zombie[i].SetDelay(x_pos);
		zombie[i].SetIsAlive(true);
	}
...
void CGameStateRun::newlevel()
{
	...
	if (level == 1) {
		const int ZOMBIE_GAP = 90;
		const int ZOMBIE_XY_OFFSET = 45;
		const int ZOMBIE_PER_ROW = 3;
		enemyconst = 5;
		ball = new CBall[enemyconst];
		for (int i = 0; i < enemyconst; i++)
		...
		for (int i = 0; i < enemyconst; i++) {				// 設定球的起始座標
			int x_pos = i % ZOMBIE_PER_ROW;
			int y_pos = i / ZOMBIE_PER_ROW;
			zombie[i].SetXY(x_pos * ZOMBIE_GAP + ZOMBIE_XY_OFFSET, y_pos * ZOMBIE_GAP + ZOMBIE_XY_OFFSET);
			zombie[i].SetDelay(x_pos);
			zombie[i].SetIsAlive(true);
		}
	}
...
void CGameStateRun::OnMove()							// 移動遊戲元素
{
	...
	if (level == 1 ) {
		for (int i = 0; i < 5; i++) {
			for (int i = 0; i < totalzombie; i++) {
				if (zombie[i].IsAlive() && zombie[i].HitCSword(&sword)) {
					enemyconst--;
					zombie[i].SetIsAlive(false);
					CAudio::Instance()->Play(AUDIO_SWORD_HIT);

				}
				if (zombie[i].IsAlive() && zombie[i].HitEraser(&eraser)) {
					enemyconst--;
					zombie[i].SetIsAlive(false);
					CAudio::Instance()->Play(AUDIO_DING);
					hits_left.Add(-1);
	...
	if (level == 1) {
		for (int i = 0; i < totalzombie; i++) {
			temp = 0;
			for (int j = 0; j < totalzombie; j++) {
				if (j != i) {
					if (zombie[i].HitOthers(&zombie[j])) {
						zombie[i].istach();
						temp = 1;
						break;
					}
			...
			if (temp == 0) {
				zombie[i].getxy(eraser.GetX1(), eraser.GetY1());
			}
